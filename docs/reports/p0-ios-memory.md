...
**转入 P1 的前置验证项**（接入真实订阅后立即执行，门槛不变）：
- [ ] 60+ 节点 + 推荐 .mrs 规则集 + 30 分钟浏览级流量下的常驻/峰值（原任务 5.2）；
- [ ] GOMEMLIMIT 25/30/35 档位扫描确定默认值（原任务 5.3，当前暂用 30MiB）；
- [ ] 代理节点出站冒烟（直连变体已通过）。
- 若超限 → 启用 §5 缓解阶梯；仍不达标 → 按 spec 另立 change 重审 P1 方案。
