...
- 版本：v1.16
- 日期：2026-06-18
- 状态：标志改版为「双 T」——经用户确认替换原双孔拱桥；已在 Figma 全套落地（App 图标 / 横版 / mark / Hero），颜色绑 `sys/brand` 变量；双 T 图标已组件化为共用 `TT mark`（v1.16）
...
### 1.2 核心意象：双 T 组成 M
标志主体是**两个字母 T**——左 T 顺时针、右 T 逆时针各旋转（横版／App 图标取 30°），镜像对称、竖向外张。该形态同时承载三层语义，叠合而不冲突：

| 层次 | 含义 | 视觉落点 |
|------|------|----------|
| 字母 TT | **通途 Tongtu** 首字母 | 两个清晰可读的 T，品牌缩写、识别度高（小尺寸仍可辨） |
| 字母 M | 致敬官方内核 **mihomo** | 两 T 竖向外张成腿、横在顶相接成中峰，整体轮廓自然读作 M |
| 代理连通 | 在两端之间建立通路 | 双 T 由两端向中相接，表达两端之间建立通路 |

> 由原「双孔拱桥」改版而来：原圆弧双拱与麦当劳金拱门高度撞形，故改为直线的双 T——既彻底避开撞图，又保留 M(mihomo) 的轮廓语义，并新增 TT(Tongtu) 的明确品牌缩写。详见 §10 v1.15。

### 1.3 元素释义
- **两个 T（唯一元素）**：每个 T = 一横 + 一竖（圆角矩形线段）；左 T 顺时针、右 T 逆时针旋转，镜像对称。横在顶部内侧相接成 M 中峰，竖向外下张开成 M 两腿。
- **圆角矩形线段**：所有笔画为圆角矩形（`rx` 约线宽的 1/5），非圆头胶囊；直线棱角，区别于原双拱的圆弧。

> 设计演进：双孔拱桥（v1.0–v1.14）→ 双 T（v1.15）。原双拱曾尝试「中墩钥石」「水中倒影」装饰层（均因易误读/削弱极简而移除），最终因与麦当劳金拱门撞形被整体替换为双 T；双 T 用直线避撞，兼顾 TT(品牌缩写) + M(mihomo 轮廓) 双重语义。详见 §10。
...
### 2.1 双 T 几何（1024 画板）
标志居中于 1024 画板。单个 T 的基准定义（旋转前，T 局部坐标）：
- **横**：圆角矩形 220 × 40，`rx` 8，位于竖顶（横中点 = 竖顶）。
- **竖**：圆角矩形 40 × 300，`rx` 8，自横中点向下。
- 两 T 中心分别置于画板中心左右 ±100、整体下移 20；**左 T 顺时针 30°、右 T 逆时针 30°**（镜像对称）。
- App 图标双 T 占画板宽 **72%**（方版／圆角版）、**65%**（圆形版——圆形视觉需更多内边距）；横版／Hero／mark 各保持其比例。
- 颜色：反白 `sys/brand/white`、单色 `sys/brand/indigo`、横版 indigo→cyan 渐变。

### 2.2 SVG 定义（双 T，居中 1024 画板）
```svg
<g transform="translate(512,512) scale(1.2)">
  <g transform="translate(-100,20) rotate(30)">      <!-- 左 T 顺时针 30° -->
    <rect x="-110" y="-20" width="220" height="40"  rx="8"/>  <!-- 横 -->
    <rect x="-20"  y="-20" width="40"  height="300" rx="8"/>  <!-- 竖 -->
  </g>
  <g transform="translate(100,20) rotate(-30)">      <!-- 右 T 逆时针 30°（镜像） -->
    <rect x="-110" y="-20" width="220" height="40"  rx="8"/>
    <rect x="-20"  y="-20" width="40"  height="300" rx="8"/>
  </g>
</g>
```
- 各形态按画板尺寸等比缩放：横版 / Hero viewBox 612×360（缩放约 1.0）、mark 324×190（缩放约 0.55）。
- **Figma 注意**：`createNodeFromSvg` 导入后 `<rect>` 转为 VECTOR（非 RECTANGLE）；颜色绑变量须遍历 VECTOR 用 `setBoundVariableForPaint`。
...
- **横版 mark**：双 T 沿**水平** `indigo → cyan` 渐变（左 indigo → 右 cyan）。
...
采用 **Material 3 token 分层**：ref（原始）→ sys（语义）。logo 作为品牌资产直接绑 `sys/brand`；**comp（组件层）留给将来真正的 UI 组件**（Button / Card 等），不为单一 logo 建 comp token（主流做法，避免过度）。

| 集合 | 层级 | 内容 |
|------|------|------|
| `Primitives` | ref | indigo/cyan/neutral/error tonal palette（35 色，scopes 隐藏）|
| `App Theme` | sys | 17 token × Light/Dark，alias Primitives（app UI 语义色）|
| `Brand Colors` | sys | logo 13 色，alias Primitives |
| `Spacing` | sys | 通用 scale（8–110）＋ 语义间距（alias scale）|
| `Size` | sys | 字号 `font`（200/72/50）＋ 尺寸 `dim`（mark-height 360、app-icon 1024）|
| `Radius` | sys | 圆角（hero 32 / icon 230 / icon-circle 512）|

- 引用链：**logo 节点 → sys（Brand Colors / Spacing / Size / Radius）→ Primitives（ref）**；单一真相源、全链联动。
- logo 各属性已绑变量：**颜色（含渐变 stop）、gap/padding、字号、圆角**。
- **透明度例外**：Hero 网络淡色用的是 paint（fill/stroke）的 opacity，Figma 变量不支持绑定 paint opacity（`OPACITY` 仅绑节点级 opacity），故保持硬编码——平台限制，非遗漏。
- 命名加 `ref/` `sys/` 层级前缀；全部变量含 WEB/iOS/Android code syntax（Dev Mode 可导出）。
- Lockup 与 Hero 顶层均为 auto-layout `[mark, text]`（Hero 多 network absolute 背景），结构一致。
- 画布「Color Tokens」区展示主要品牌色板。
- **双 T 图标已组件化**：Figma 建共用 component `TT mark`（归画布 `Components` 区）；App 图标方/圆角/圆形、mark 单色/反白、Lockup、Hero 共 **7 处双 T 均为其 instance**，各自 override 颜色（白/靛蓝/渐变）与 scale——改双 T 形状只改 master 一处、7 处联动。
...
| 代号 | 组合 | 用途 |
|------|------|------|
| A · App 图标 | 渐变底板 + 反白双 T（方版／圆角版／圆形遮罩版） | iOS/iPadOS/macOS/Android 应用图标 |
| B · 横版锁定 | 渐变 mark ＋「通途」＋「Tongtu」＋ slogan | README、官网、关于页、文档头 |
| C · 纯 mark | 单色 / 反白双 T | favicon、加载占位、超小标识、圆形遮罩 |
...
深色横幅，以双 T（M 轮廓）为焦点、节点网络铺于深色背景，营造「网络连接」氛围。用于官网 Hero、关于页、加载动画、文档头图、社交封面。
- **底**：深靛→深青对角渐变 `#1B1E40 → #0A2A33`（呼应主渐变「靛→青」走向）。
- **焦点**：双 T 反白。
- **网络**：淡靛圆环（`#7E92E0`）＋ 青色实点（`#00BCD4`）＋ 淡白点 ＋ 淡连接线（`#6478C8`），作背景层，**不得盖过主 logo 与文字**。
- **铁律**：主 logo（双 T）始终是焦点；密集网络是独立背景层、只作辅助，**不可用作主 App 图标**。
...
- **最小尺寸**：App 图标 ≥ 24 px 仍可辨双 T；< 24 px 用纯 mark 单色版。横版整体宽 ≥ 120 px，更窄去 slogan，再窄仅用 mark。
- **超小尺寸（≤16 px）**：双 T 笔画可适当加粗、竖不分段以保清晰；双 T 本身小尺寸即可辨（48px 实测清晰可读 TT）。
...
| v1.13 | 2026-06-17 | 规范 review 整理：修复 App 图标双拱 stroke 漏绑；变量加 ref/sys/comp 层级前缀、Size 分 font/dim；115 变量补全三平台 code syntax；Spacing 语义间距 alias 引用 scale。 |
| v1.14 | 2026-06-17 | 规范取舍：移除 logo 的 comp 层（删 Component 集合），节点重绑回 sys/brand（comp 留给将来 UI 组件，更合主流）；新增 Radius 变量并绑定圆角；修复网络透明度在重绑中丢失的问题（paint opacity 无法绑变量，属平台限制）。 |
| v1.15 | 2026-06-18 | **标志改版：双孔拱桥 → 双 T**。原圆弧双拱与麦当劳金拱门撞形，改为两个字母 T（左顺时针、右逆时针各旋转 30°，镜像对称、竖外张、圆角矩形线段），兼得 TT(通途品牌缩写) + M(mihomo 轮廓) 双重语义、彻底避撞。Figma 全套替换（App 图标 方/圆角/圆形、mark 单色/反白、横版 Lockup、Network Hero），颜色绑回 `sys/brand` 变量，背景网络层保留；§1–§2 概念与制图同步重写。 |
| v1.16 | 2026-06-18 | 双 T 图标**组件化** + App 图标放大。Figma 建共用 component `TT mark`（归画布 `Components` 区），7 处双 T（App 图标方/圆角/圆形、mark 单色/反白、Lockup、Hero）改为其 instance——改 master 一处、7 处联动，颜色/scale 各 instance 独立保留。App 图标双 T 占比由 ~60% 放大：方/圆角 **72%**、圆形 **65%**（圆形留更多内边距）。§2.1、§3.5 同步。 |
